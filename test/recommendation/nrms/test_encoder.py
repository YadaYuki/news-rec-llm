...
def test_user_encoder() -> None:
    ...
    plm_news_encoder = PLMBasedNewsEncoder(pretrained, multihead_attn_num_heads, additive_attn_hidden_dim, batch_first)

    batch_size, seq_len, emb_dim = 20, 10, AutoConfig.from_pretrained(pretrained).hidden_size

    news_histories = torch.arange(batch_size * hist_size * seq_len).view(batch_size, hist_size, seq_len)

    user_encoder = UserEncoder(emb_dim, multihead_attn_num_heads, additive_attn_hidden_dim, batch_first)
